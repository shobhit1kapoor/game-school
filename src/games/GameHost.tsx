...
function LearningLaunch({ level, onStart }: { level: LevelDefinition; onStart: () => void }) {
  ...
  return <section className="lesson-launch">
    <div className="lesson-launch-top"><span className="lesson-launch-icon" aria-hidden="true">{level.subject === 'math' ? '🔢' : level.subject === 'english' ? '📚' : level.subject === 'science' ? '🔎' : '🗺️'}</span><div><p className="overline">LEARN BEFORE YOU PLAY</p><h3>Your mini-lesson</h3><p>{goal ?? `We will learn about ${level.concept.toLowerCase()}.`}</p></div></div>
    <div className="lesson-step-grid"><article><span>1</span><h4>Big idea</h4><p>{coach}</p></article><article><span>2</span><h4>See an example</h4><p>{example ?? 'Take one slow look at the clues. You can replay this page any time.'}</p></article></div>
    ...
    <div className="lesson-launch-footer"><p>When you are ready, use what you learned in all 10 practice parts. There is no timer.</p><button className="primary-button" onClick={onStart}>Start 10-part practice</button></div>
  </section>;
}
