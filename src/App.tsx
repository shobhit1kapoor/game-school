...
export default function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="main-content">
      ...
      {view === 'subjects' && <SubjectView subject={subject} levels={levels} state={state} onSubject={setSubject} onPlay={setActiveLevel} />}
...
function LandingPage({ onEnter }: { onEnter: () => void }) {
  ...
  return <main className="landing-page">
    ...
    <section className="landing-hero"><img src={landingHero} alt="Children and an owl guide walking toward Brightpath learning worlds" /><div className="landing-hero-copy"><p>Learning feels like an adventure.</p><h1>Every curious kid deserves a bright path.</h1><p className="landing-lede">A playful K–5 learning world where children explore subjects, understand an idea, practice at their own pace, and celebrate each small win.</p><div className="landing-actions"><button className="landing-primary" onClick={onEnter}>Start exploring <Play size={18} fill="currentColor" /></button><button className="landing-secondary" onClick={scrollToHow}>See how it works</button></div><div className="landing-trust"><span>✦ No ads</span><span>✦ Local progress</span><span>✦ Made for K–5</span></div></div></section>
    <section className="landing-intro" id="how-it-works"><p>Brightpath makes the next step feel clear.</p><h2>One calm journey from “I wonder” to “I can do this.”</h2><div className="landing-steps"><article><span>01</span><strong>Choose a world</strong><p>Pick a grade and subject that feels exciting today.</p></article><article><span>02</span><strong>Learn first</strong><p>Every lesson begins with a friendly explanation and example.</p></article><article><span>03</span><strong>Play and grow</strong><p>Practice in ten parts, unlock stars, and celebrate progress.</p></article></div></section>
    <section className="landing-subjects" id="subjects"><div><p>Four worlds. So many discoveries.</p><h2>Big ideas, made wonderfully approachable.</h2></div><div className="landing-subject-list">{(Object.keys(SUBJECTS) as SubjectId[]).map((id) => <button key={id} className={`landing-subject ${id}`} onClick={onEnter}><span>{SUBJECTS[id].emoji}</span><strong>{SUBJECTS[id].subjectName}</strong><small>{SUBJECTS[id].label}</small><i>Explore →</i></button>)}</div></section>
...
function SubjectView({ subject, levels, state, onSubject, onPlay }: { subject: SubjectId; levels: LevelDefinition[]; state: QuestwoodState; onSubject: (value: SubjectId) => void; onPlay: (level: LevelDefinition) => void }) { const info = SUBJECTS[subject]; return <div className="subject-page" style={{ '--math-level-covers': `url(${mathLevelCovers})`, '--math-lesson-covers': `url(${mathLessonCovers})`, '--english-lesson-covers': `url(${englishLessonCovers})`, '--science-lesson-covers': `url(${scienceLessonCovers})`, '--social-lesson-covers': `url(${socialLessonCovers})`, '--arcade-scenes': `url(${arcadeScenes})` } as React.CSSProperties}><div className="subject-header" style={{ '--subject-color': info.color } as React.CSSProperties}><span>{info.emoji}</span><div><p className="overline">YOUR SUBJECT DISTRICT</p><h1>{info.subjectName} · {info.label}</h1><p>{info.description} Pick any level below; you are free to learn in the order that feels right.</p></div></div><div className="subject-tabs">{(Object.keys(SUBJECTS) as SubjectId[]).map((id) => <button className={subject === id ? 'selected' : ''} onClick={() => onSubject(id)} key={id}>{SUBJECTS[id].emoji} {SUBJECTS[id].subjectName}</button>)}</div><section className="stage-list">{STAGES[subject].map((stage, stageIndex) => <section className="stage-section" key={stage.name}><header><div><span>Stage {stageIndex + 1}</span><h2>{stage.name}</h2><p>{stage.mission}</p></div><p className="stage-skills">{stage.skills.join(' · ')}</p></header><div className="level-rail">{levels.slice(stageIndex * 2, stageIndex * 2 + 2).map((level, offset) => { const index = stageIndex * 2 + offset; const current = levelState(level, state); const mastery = state.mastery[level.skillId]?.score ?? 0; const cover = subject === 'math' ? `math-lesson-cover lesson-${index}` : subject === 'english' ? `english-lesson-cover lesson-${index}` : subject === 'science' ? `science-lesson-cover lesson-${index}` : `social-lesson-cover lesson-${index}`; return <article className={`level-card ${current}`} key={level.id}><i className={`level-cover ${cover}`} /><div className="level-card-content"><div className="level-number">{current === 'complete' ? '✓' : index + 1}</div><p className="overline">Lesson {index + 1} · 10 parts</p><h2>{level.title}</h2><h3>{level.concept}</h3><p>{current === 'complete' ? `Mastery: ${mastery}%` : 'Interactive mini-lesson, then 10 practice parts.'}</p><button className={current === 'complete' ? 'secondary-button' : 'primary-button'} onClick={() => onPlay(level)}>{current === 'complete' ? 'Play again' : <><Play size={17} fill="currentColor" /> Learn &amp; play</>}</button></div></article>; })}</div></section>)}</section></div>; }
