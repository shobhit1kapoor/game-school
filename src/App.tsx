...
import { enableGameSchoolClickSounds, playSound, setAmbientTheme, stopAmbientMusic, type AmbientTheme } from './lib/sound';
...
export default function App() {
  ...
  const [music, setMusic] = useState<AmbientTheme>('Quiet forest');
  ...
  useEffect(() => enableGameSchoolClickSounds(), []);
  ...
  const finishLevel = (result: GameResult) => {
    ...
    setNotice(completesGrade && nextGrade ? `Grade ${activeLevel.grade} is complete! Welcome to Grade ${nextGrade}.` : completesGrade ? 'You finished Grade 5! You completed the Game School journey.' : earned ? `Wonderful work! You earned ${earned} stars for learning and completing this lesson.` : 'Good exploring. Try this level again whenever you feel ready.');
    setActiveLevel(null);
  };
  const chooseSubject = (next: SubjectId) => { window.scrollTo(0, 0); setSubject(next); setView('subjects'); };
  const navigate = (next: View) => { window.scrollTo(0, 0); setView(next); if (next === 'games') setArcadeSubject(null); };
  const enterAcademy = (nextSubject?: SubjectId) => { window.scrollTo(0, 0); if (nextSubject) { setSubject(nextSubject); setView('subjects'); } else setView('home'); setShowAcademy(true); };
  const learnLesson = (level: LevelDefinition) => { playSound('reward'); setState((current) => markLessonLearned(current, level)); };

  if (!showAcademy) return <LandingPage onEnter={enterAcademy} />;

  return <main className="app-shell">
    <aside className="sidebar">
      <div className="brand"><span className="brand-mark"><img src={gameSchoolLogo} alt="" /></span><span>Game<br />School</span></div>
      <nav aria-label="Main navigation">{NAVIGATION.map(({ id, label, icon: Icon }) => <button className={view === id ? 'nav-item active' : 'nav-item'} onClick={() => navigate(id)} key={id}><Icon size={23} /><span>{label}</span></button>)}</nav>
      <div className="sidebar-bottom"><div className="safe-note"><ShieldCheck size={18} /><span>Local progress<br />saved on this device</span></div><button className="help-button" onClick={() => setShowHelp(true)}><CircleHelp size={19} /> Help</button></div>
    </aside>
    <section className="main-content">
      <header className="topbar"><label className="grade-picker">Grade <select value={grade} onChange={(event) => switchGrade(event.target.value as Grade)}>{GRADES.map((item) => <option key={item} value={item}>{item === 'K' ? 'Kindergarten' : item}</option>)}</select><ChevronDown size={17} /></label><div className="profile-bar"><label className="music-picker"><Music2 size={18} /><select aria-label="Background music" value={music} onChange={(event) => { const next = event.target.value as AmbientTheme; setMusic(next); setAmbientTheme(next); }}><option>Quiet forest</option><option>Mountain breeze</option><option>Ocean sparkle</option><option>Music off</option></select></label><button className="profile-edit" onClick={() => setEditingProfile(true)}><span className="avatar">{state.profile.avatar}</span><strong>{state.profile.name}</strong></button><span className="star-count"><Star size={18} fill="currentColor" /> {state.profile.stars} stars</span></div></header>
...
function LandingPage({ onEnter }: { onEnter: (subject?: SubjectId) => void }) {
  const scrollToHow = () => document.getElementById('how-it-works')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  return <main className="landing-page">
    <header className="landing-nav"><button className="landing-brand" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} aria-label="Back to top"><img src={gameSchoolLogo} alt="" /><span>Game<br />School</span></button><nav aria-label="Landing navigation"><button onClick={scrollToHow}>How it works</button><button onClick={() => document.getElementById('subjects')?.scrollIntoView({ behavior: 'smooth' })}>Subjects</button></nav><button className="landing-login" onClick={() => onEnter()}>Enter Game School <Play size={16} fill="currentColor" /></button></header>
    <section className="landing-hero"><img src={landingHero} alt="Children and an owl guide walking toward Game School learning worlds" /><div className="landing-hero-copy"><p>Learning feels like an adventure.</p><h1>A K–5 school built like a world to explore.</h1><p className="landing-lede">Game School turns school into a clear, child-friendly journey: choose a grade, enter a subject world, learn one concept at a time, then show what you know through playful practice and games.</p><div className="landing-actions"><button className="landing-primary" onClick={() => onEnter()}>Start exploring <Play size={18} fill="currentColor" /></button><button className="landing-secondary" onClick={scrollToHow}>See how Game School works</button></div><div className="landing-trust"><span>✦ Kindergarten–Grade 5</span><span>✦ 240 lessons</span><span>✦ 14 local games</span><span>✦ No ads</span></div></div></section>
    <section className="landing-intro" id="how-it-works"><p>Game School makes the next step feel clear.</p><h2>A school journey with stages, lessons, and game checkpoints.</h2><p className="landing-detail">Each grade is a school stage. Inside every stage, children choose Math, English, Science, or Social Studies. Every subject has 10 lessons; each lesson is a class with a guided explanation, a worked example, and 10 practice parts. There are no traditional exams—students demonstrate growth by completing an end-of-stage game and building a personal high score.</p><div className="landing-steps"><article><span>01</span><strong>Choose a grade</strong><p>Start in Kindergarten or Grades 1–5. Content, vocabulary, and challenge rise with the learner.</p></article><article><span>02</span><strong>Learn one lesson</strong><p>Each lesson explains one idea first, then offers a calm 10-part interactive practice round.</p></article><article><span>03</span><strong>Play the checkpoint</strong><p>Complete a stage, play a subject game, earn a high score, stars, badges, and leaderboard progress.</p></article></div></section>
    <section className="landing-subjects" id="subjects" style={{ '--landing-subject-worlds': `url(${landingSubjectWorlds})` } as React.CSSProperties}><div><p>Four worlds. So many discoveries.</p><h2>Every subject is a place children want to visit.</h2><span className="landing-section-note">Explore Math Mountain, Story Forest, Discovery Island, and Community Town. More local game adventures are being added.</span></div><div className="landing-subject-list">{(Object.keys(SUBJECTS) as SubjectId[]).map((id) => <button key={id} className={`landing-subject ${id}`} onClick={() => onEnter(id)}><i className="landing-subject-art" aria-hidden="true" /><span>{SUBJECTS[id].emoji}</span><strong>{SUBJECTS[id].subjectName}</strong><small>{SUBJECTS[id].label}</small><em>{id === 'math' ? 'Numbers, patterns, and problem-solving' : id === 'english' ? 'Reading, writing, and language' : id === 'science' ? 'Experiments, nature, and discovery' : 'Maps, communities, and history'}</em><b>Explore this world →</b></button>)}</div></section>
    <section className="landing-cta"><div><p>Ready when they are.</p><h2>Let curiosity lead the way.</h2><span>Start with one world. There is always another adventure waiting.</span></div><button className="landing-primary" onClick={() => onEnter()}>Enter Game School <Play size={18} fill="currentColor" /></button></section>
    <footer className="landing-footer"><span>© Game School</span><span>Learning made with wonder.</span></footer>
  </main>;
}

const arcadeUrl = (path: string) => `${import.meta.env.BASE_URL}${path.replace(/^\//, '')}`;

const ARCADE_GAMES: Record<SubjectId, Array<{ title: string; note: string; href?: string; art?: number; cover?: 'math' | 'arcade' | 'social' }>> = {
  math: [{ title: 'Cannon Target', note: 'Aim at the right number', art: 0, cover: 'math' }, { title: 'Number Road', note: 'Drive to the right answer', href: arcadeUrl('arcade/number-road/index.html'), art: 1, cover: 'math' }, { title: 'Puzzle Playground', note: 'Solve colorful math puzzles', href: arcadeUrl('arcade/mathivities/index.html'), art: 2, cover: 'math' }, { title: 'Deep Sea Numbers', note: 'Explore number oceans', href: arcadeUrl('arcade/ocean-math-quest/index.html'), art: 3, cover: 'math' }, { title: 'Multiply Island', note: 'A multiplication treasure trip', href: arcadeUrl('arcade/multiply-island/index.html'), art: 4, cover: 'math' }, { title: 'Number Nest', note: 'Light stars by solving number puzzles', href: arcadeUrl('arcade/number-nest/index.html'), art: 5, cover: 'math' }],
  science: [{ title: 'Water Workshop', note: 'Build and test tiny experiments', href: arcadeUrl('arcade/flowspark-kids/index.html'), art: 2, cover: 'arcade' }, { title: 'Space Explorer', note: 'English version coming soon' }, { title: 'Water Drop Journey', note: 'Travel through the water cycle', href: arcadeUrl('arcade/follow-the-drop/index.html'), art: 3, cover: 'arcade' }, { title: 'Science Mix Lab', note: 'Mix, discover, and learn', href: arcadeUrl('arcade/chemicraft/index.html'), art: 4, cover: 'arcade' }, { title: 'Animal Safari', note: 'Meet animals from around the world' }, { title: 'Space Pals', note: 'A space science adventure' }],
  social: [{ title: 'History Delivery', note: 'Carry important messages through time', href: arcadeUrl('arcade/time-travel-courier/index.html'), art: 0, cover: 'social' }, { title: 'Community Choices', note: 'Learn how a town works' }, { title: 'World Chase', note: 'Discover places and maps', href: arcadeUrl('arcade/world-chase/index.html'), art: 1, cover: 'social' }, { title: 'Around the World', note: 'Explore countries and landmarks' }, { title: 'Time Trail', note: 'Put history in order', href: arcadeUrl('arcade/time-trail/index.html'), art: 2, cover: 'social' }, { title: 'Secret History Codes', note: 'Solve history puzzles', href: arcadeUrl('arcade/secret-history-codes/index.html'), art: 3, cover: 'social' }],
  english: [{ title: 'Word Wizard', note: 'Spell words in magical worlds', href: arcadeUrl('arcade/wordquest/index.html'), art: 0, cover: 'arcade' }, { title: 'Reading Trail', note: 'A phonics reading journey' }, { title: 'Sound & Picture', note: 'Match first sounds to pictures', href: arcadeUrl('arcade/pic-phonics/index.html'), art: 1, cover: 'arcade' }, { title: 'Word Garden', note: 'Grow your vocabulary' }, { title: 'Grammar Castle', note: 'Build strong sentences' }, { title: 'Story Adventure', note: 'Read and explore stories' }],
};
...
function LeaderboardView({ profile }: { profile: QuestwoodState['profile'] }) { const rank = rankingsFor(profile).findIndex((entry) => entry.current) + 1; return <div className="simple-page leaderboard-page"><p className="overline">YOUR FRIENDLY RANKING</p><h1>Leaderboard</h1><p className="intro">Celebrate progress with a friendly local demo. Your position changes as you earn lesson stars.</p><div className="leaderboard-spotlight"><span className="rank-avatar">{profile.avatar}</span><div><strong>{profile.name}, you are in place #{rank}</strong><p>Keep completing lessons to collect stars and climb the friendly board.</p></div><span><Star fill="currentColor" /> {profile.stars}</span></div><FriendlyLeaderboard profile={profile} /></div>; }

function GrownupsView({ state, onReset }: { state: QuestwoodState; onReset: () => void }) { const records = Object.values(state.mastery); const completed = Object.values(state.progress).filter((item) => item.state === 'complete').length; const average = records.length ? Math.round(records.reduce((total, record) => total + record.score, 0) / records.length) : 0; return <div className="simple-page grownups"><p className="overline">LOCAL GROWN-UPS VIEW</p><h1>Learning progress for {state.profile.name}</h1><p className="intro">This local view shows real saved practice from this device. There are no accounts, ads, or trackers.</p><section className="grownup-summary" aria-label="Learning summary"><article><span>📚</span><strong>{completed}</strong><small>lessons complete</small></article><article><span>⭐</span><strong>{state.profile.stars}</strong><small>stars collected</small></article><article><span>🌱</span><strong>{average}%</strong><small>average mastery</small></article></section><div className={records.length === 0 ? 'mastery-list empty-mastery' : 'mastery-list'}>{records.length === 0 ? <><span aria-hidden="true">🗺️</span><strong>The first learning trail is waiting.</strong><p>Complete any lesson and its saved mastery will appear here.</p></> : records.map((record) => <div key={record.skillId}><div><strong>{record.skillId.replaceAll('.', ' · ')}</strong><span>{record.score}% mastery</span></div><progress max="100" value={record.score} /></div>)}</div><button className="danger-button" onClick={onReset}>Reset local demo profile</button></div>; }
